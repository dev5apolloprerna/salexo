...
class AdminFollowupsCron extends Command
{

    protected $signature = 'admin_followup:cron';
    protected $description = 'Command description';

    public function handle()
    {
        Log::info('Admin Followup Cron Executed');

        // Get all employees
        $employees = Employee::select('employee_master.*', 'company_client_master.company_name')
            ->where('employee_master.isCompanyAdmin', 1)
        ...
        foreach ($employees as $employee) 
        {
            ...
            $allLeads = LeadMaster::where('iCustomerId', $employee->company_id)
            ->where(['iStatus' => 1, 'isDelete' => 0])
            ->get();
