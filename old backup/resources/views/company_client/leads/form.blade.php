...
<div class="row">
    <div class="col-lg-4 col-md-6 mt-3">
        ...
        @php
          $psId   = old('product_service_id', $lead->product_service_id ?? null);
          $psOtherVal = old('product_service_other', $lead->product_service_other ?? '');
          $psIsOther  = !$psId && $psOtherVal;
        @endphp
        <select name="product_service_id" id="product_service_id" class="form-control" required>
            ...
            @endforeach
            <option value="other" {{ $psIsOther || old('product_service_id')==='other' ? 'selected' : '' }}>Other</option>
        </select>
        
        <div id="product_service_other_box" class="mt-2" style="display:none;">
          <input type="text" class="form-control" name="product_service_other"
                 id="product_service_other" maxlength="255"
                 placeholder="Please specify service/product"
                 value="{{ $psOtherVal }}" autocomplete="off">
        </div>
    </div>
    ...
    <div class="col-lg-4 col-md-6 mt-3">
        ...
        @php
          $lsId   = old('LeadSourceId', $lead->LeadSourceId ?? null);
          $lsOtherVal = old('LeadSource_other', $lead->LeadSource_other ?? '');
          $lsIsOther  = !$lsId && $lsOtherVal;
        @endphp
        <select name="LeadSourceId" id="LeadSourceId" class="form-control" required>
            ...
            @foreach ($leadSources as $id => $source)
                <option value="{{ $id }}"
                    ...
                    {{ $source }}</option>
            @endforeach
            <option value="other" {{ $lsIsOther || old('LeadSourceId')==='other' ? 'selected' : '' }}>Other</option>
        </select>
        
        <div id="LeadSource_other_box" class="mt-2" style="display:none;">
          <input type="text" class="form-control" name="LeadSource_other"
                 id="LeadSource_other" maxlength="255"
                 placeholder="Please specify lead source"
                 value="{{ $lsOtherVal }}" autocomplete="off">
        </div>
    </div>
        ...
        <select class="form-control" name="employee_id" id="employee_id" required>
            ...
            @foreach ($employees as $employee)
                <option value="{{ $employee->emp_id }}"
                    {{ isset($lead) && $lead->employee_id == $employee->emp_id ? 'selected' : '' }}>
                    {{ $employee->emp_name }}
                </option>
            @endforeach
        </select>
...
@foreach ($udf as $udfs)
    @php
        $value = $leadUdfData[$udfs->id] ?? '';
    @endphp

    <div class="col-lg-4 col-md-6 mt-3">
        {{ $udfs->label }} <span class="text-danger"> {{ $udfs->required == 'Yes' ? '*' : '' }}</span>
        <input type="text" id="{{ $udfs->label }}" name="udf[{{ $udfs->id }}]" class="form-control"
            placeholder="Enter {{ $udfs->label }}" value="{{ old('udf.' . $udfs->id, $value) }}"
            {{ $udfs->required == 'Yes' ? 'required' : '' }}>
    </div>
@endforeach
