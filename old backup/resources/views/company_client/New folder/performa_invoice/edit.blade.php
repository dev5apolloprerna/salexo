          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title mb-0">
                Edit Invoice
                <a href="{{ route('invoice.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" style="float:right;">
                  Back
                </a>
              </h5>
              <hr>
              ...
              <div class="live-preview">
                <form method="POST" action="{{ route('invoice.update', $Data->invoiceId) }}" enctype="multipart/form-data">
                  @csrf
                  @method('PUT')
                  ...
                  <div class="card-body">
                    <div class="form-group row">
                     ...
                     <!--  <div class="col-sm-6 mb-3 mt-3 mb-sm-0">
                      ...
                      <div class="col-sm-6 mb-3 mt-3 mb-sm-0">
                        <label for="iInvoiceNo"><span style="color:red;">*</span> Invoice No</label>
                        <input
                          class="form-control @error('iInvoiceNo') is-invalid @enderror"
                          id="iInvoiceNo"
                          name="iInvoiceNo"
                          type="text"
                          placeholder="Enter Quotation No"
                          value="{{ old('iInvoiceNo', $Data->iInvoiceNo) }}"
                          readonly
                        >
                        @error('iInvoiceNo')<div class="invalid-feedback">{{ $message }}</div>@enderror
                      </div>
                  ...
                  <div class="card-footer">
                    ...
                    <a class="btn btn-primary float-right mr-3 mb-3" href="{{ route('invoice.index') }}">Cancel</a>
                  </div>
                </form>
              </div> {{-- live-preview --}}
            </div>
...
@section('scripts')
  ...
  /*  $('#mappingCompany').on('change', function () {
      ...
      if (companyId) {
        $.ajax({
          url: "{{ route('invoice.getNextNo', ':companyId') }}".replace(':companyId', companyId),
          type: 'GET',
          success: function (data) {
            $('input[name="iInvoiceNo"]').val(data);
          },
          error: function (xhr) { console.error(xhr.responseText); }
        });
      } else {
        $('input[name="iInvoiceNo"]').val('');
      }
    });*/
 ...
 $('#EditcompanyID').change(function() {
        ...
        function mapping() {
            ...
            var url = "{{ route('invoice.mapping', ':company') }}";
        ...
        const QuotationNo = $('#iInvoiceNo').val();
        ...
        function setQuotationNo() 
        {
          ...
          {
          $.ajax({
            url: "{{ route('invoice.getNextNo', ':companyId') }}".replace(':companyId', COMPANY_ID),
            type: 'GET',
            success: function (data) {
              $('input[name="iInvoiceNo"]').val(data);
            },
