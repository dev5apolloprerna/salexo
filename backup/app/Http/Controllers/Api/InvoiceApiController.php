   ...
   public function index(Request $request)
    {
        ...
        $detailsSub = DB::table('invoicedetails')
            ->select('InvoiceID', DB::raw('COUNT(*) as product_count'))
            ->where([
                'isDelete' => 0,
                'iStatus'  => 1,
            ])
        ...
        $query = Invoice::query()
            ->where(['invoice.iStatus' => 1, 'invoice.isDelete' => 0,'invoice.iCompanyId'=>$user->company_id])
        ...
        $query = $query
            ...
            ->leftJoinSub($detailsSub, 'qd', function ($join) {
                $join->on('qd.InvoiceID', '=', 'invoice.InvoiceId');
            })
            ->orderByDesc('invoice.InvoiceId')
            ->select([
                'invoice.*',
    ...
    public function store(Request $request)
    {
        ...
        $v = Validator::make($request->all(), [
            ...
            'iInvoiceNo' => [
                ...
                Rule::unique('invoice', 'iInvoiceNo')
                    ->where(fn ($q) => $q
        ...
        $data = [
            ...
            'iInvoiceNo'      => (string) $request->iInvoiceNo,
        ...
        return response()->json([
            ...
            'Invoice_id' => $id,
            'redirect_to_details' => route('api.invoice.details', $id)
        ], 201);
    }
    ...
    {
        $q = Invoice::where(['invoice.iStatus' => 1, 'invoice.isDelete' => 0, 'invoice.InvoiceId' => $id])
            ->join('company_client_master', 'invoice.iCompanyId', '=', 'company_client_master.company_id')
            ->join('party', 'invoice.iPartyId', '=', 'party.partyId')
            ->join('year', 'invoice.iYearId', '=', 'year.year_id')
            ->select([
    ...
    public function update(Request $request, $id)
    {
        ...
        $v = Validator::make($request->all(), [
            ...
            'iInvoiceNo' => [
                'required', 'string', 'max:50',
                Rule::unique('invoice', 'iInvoiceNo')
                    ->ignore($id, 'InvoiceId')                // ignore current row
                    ->where(fn($q) => $q
                        ->where('iCompanyId', $request->iCompanyId ?? optional($request->user())->company_id)
                        ->where('iYearId',    $request->iYearId)
                        ->where('isDelete',   0)
    ...
    public function details($id)
    {
        $header = Invoice::select(
                ...
                'invoice.modeOfDespatch',
                'invoice.deliveryTerm',
                'invoice.paymentTerms',
                'invoice.iGstType',
                'invoice.strTermsCondition',
                'invoice.InvoiceId'
            )
            ->where(['invoice.iStatus' => 1, 'invoice.isDelete' => 0, 'invoice.InvoiceId' => $id])
            ->join('company_client_master', 'invoice.iCompanyId', '=', 'company_client_master.company_id')
            ->join('party', 'invoice.iPartyId', '=', 'party.partyId')
            ->join('year', 'invoice.iYearId', '=', 'year.year_id')
            ->first();
        ...
        $items = InvoiceDetail::from('invoicedetails')
            ->where([
                'invoicedetails.iStatus'    => 1,
                'invoicedetails.isDelete'   => 0,
                'invoicedetails.InvoiceID'=> $id,
            ])
            ->join('service_master', 'service_master.service_id', '=', 'invoicedetails.productID')
    ...
    public function pdf($id)
    {
        ...
        $items = InvoiceDetail::where([
            'invoicedetails.iStatus'  => 1,
            'invoicedetails.isDelete' => 0,
            'invoicedetails.InvoiceID' => $id
        ])->orderBy('invoicedetailsId')->get();
        ...
        $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])
            ->loadview('company_client.invoice.detailPDF', [
                'Invoice'       => $q,
                'InvoiceDetail' => $items,
                'TermCondition'   => $terms,
        ...
        $downloadName = $q->strPartyName . $q->iInvoiceNo . '.pdf';
    ...
    public function copy($id)
    {
        ...
        $last = Invoice::where('iCompanyId', $old->iCompanyId)
        ...
        // ✅ INSERT NEW Invoice
        $copyId = DB::table('invoice')->insertGetId([
            'iYearId'           => $old->iYearId,
