...
class PerformaInvoiceApiController extends Controller
{
    /**
     * GET /api/Invoices
   ...
   public function index(Request $request)
    {
        ...
        // Subquery to count products per Invoice
        $detailsSub = DB::table('performa_invoicedetails')
            ->select('performainvoiceID', DB::raw('COUNT(*) as product_count'))
            ->where([
                'isDelete' => 0,
                'iStatus'  => 1,
            ])
            ->groupBy('performainvoiceID');
    
        $query = PerformaInvoice::query()
            ->where(['performa_invoice.iStatus' => 1, 'performa_invoice.isDelete' => 0,'performa_invoice.iCompanyId'=>$user->company_id])
        ...
        $query = $query
            ...
            ->leftJoinSub($detailsSub, 'qd', function ($join) {
                $join->on('qd.performainvoiceID', '=', 'performa_invoice.performainvoiceID');
            })
            ->orderByDesc('performa_invoice.performainvoiceID')
            ->select([
                'performa_invoice.*',
    ...
    public function getNextInvoiceNo()
    {
        ...
        $next = str_pad((string)($n + 1), 4, '0', STR_PAD_LEFT);
            ...
            $nextInvoiceNo=$next . '/' . $year;
            return response()->json(['next_performa_invoice_no' => $nextInvoiceNo] );
    }
    ...
    public function store(Request $request)
    {
        ...
        $v = Validator::make($request->all(), [
            ...
            'iPerformaInvoiceNo' => [
                ...
                Rule::unique('performa_invoice', 'iPerformaInvoiceNo')
                    ->where(fn ($q) => $q
            ...
            'InvoiceValidity'  => 'nullable|string|max:255',
        ...
        $id = DB::table('performa_invoice')->insertGetId($data);

        return response()->json([
            'success'=>true,
            'message' => 'Performa Invoice created successfully',
            'Invoice_id' => $id,
            'redirect_to_details' => route('api.performa_invoice.details', $id)
        ], 201);
    }

    /**
     * GET /api/Invoices/{id}
     * Show a Invoice (basic)
     */
    public function show($id)
    {
        $q = PerformaInvoice::where(['performa_invoice.iStatus' => 1, 'performa_invoice.isDelete' => 0, 'performa_invoice.performainvoiceID' => $id])
            ->join('company_client_master', 'performa_invoice.iCompanyId', '=', 'company_client_master.company_id')
            ->join('party', 'performa_invoice.iPartyId', '=', 'party.partyId')
            ->join('year', 'performa_invoice.iYearId', '=', 'year.year_id')
            ->select([
    ...
    public function update(Request $request, $id)
    {
        ...
        $v = Validator::make($request->all(), [
            ...
            'iPerformaInvoiceNo' => [
                'required', 'string', 'max:50',
                Rule::unique('performa_invoice', 'iPerformaInvoiceNo')
                    ->ignore($id, 'performainvoiceID')                // ignore current row
                    ->where(fn($q) => $q
                        ->where('iCompanyId', $request->iCompanyId ?? optional($request->user())->company_id)
                        ->where('iYearId',    $request->iYearId)
                        ->where('isDelete',   0)
        ...
        $affected = DB::table('performa_invoice')
    ...
    {
        $detail=PerformaInvoiceDetail::where(['performainvoiceID'=>$id])->delete();
        $deleted = DB::table('performa_invoice')
            ->where(['iStatus' => 1, 'isDelete' => 0, 'performainvoiceID' => $id])
            ->delete();
        ...
        return response()->json(['success'=>true,'message' => 'Performa Invoice deleted successfully']);
    }
    ...
    public function details($id)
    {
        $header = PerformaInvoice::select(
                ...
                'performa_invoice.iCompanyId',
                'performa_invoice.InvoiceValidity',
                'performa_invoice.modeOfDespatch',
                'performa_invoice.deliveryTerm',
                'performa_invoice.paymentTerms',
                'performa_invoice.iGstType',
                'performa_invoice.strTermsCondition',
                'performa_invoice.performainvoiceID'
            )
            ->where(['performa_invoice.iStatus' => 1, 'performa_invoice.isDelete' => 0, 'performa_invoice.performainvoiceID' => $id])
            ->join('company_client_master', 'performa_invoice.iCompanyId', '=', 'company_client_master.company_id')
            ->join('party', 'performa_invoice.iPartyId', '=', 'party.partyId')
            ->join('year', 'performa_invoice.iYearId', '=', 'year.year_id')
            ->first();
        ...
        $items = PerformaInvoiceDetail::from('performa_invoicedetails')
            ->where([
                'performa_invoicedetails.iStatus'    => 1,
                'performa_invoicedetails.isDelete'   => 0,
                'performa_invoicedetails.performainvoiceID'=> $id,
            ])
            ->join('service_master', 'service_master.service_id', '=', 'performa_invoicedetails.productID')
            ->orderBy('performainvoicedetailsId')
            ->get([
                'performa_invoicedetails.*',
        ...
        $invoicedetails = $header->toArray();
        $invoicedetails['sub_total']              = round($subTotal, 2);
        $invoicedetails['total_discount']         = round($discountT, 2);
        $invoicedetails['gst_total']              = round($gstTotal, 2);     // ← correct GST (on net)
        $invoicedetails['taxable_after_discount'] = round($taxableAfter, 2);
        $invoicedetails['total_amount']           = round($grandTotal, 2);
        ...
        return response()->json([
            'success'           => true,
            'message'           => 'Performa Invoice details',
            'Invoice_details' => $invoicedetails,          // includes sub_total, gst_total, total_amount
    ...
    public function pdf($id)
    {
        ...
        $items = PerformaInvoiceDetail::where([
            'performa_invoicedetails.iStatus'  => 1,
            'performa_invoicedetails.isDelete' => 0,
            'performa_invoicedetails.performainvoiceID' => $id
        ])->orderBy('performainvoicedetailsId')->get();
        ...
        $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])
            ->loadview('company_client.performa_invoice.detailPDF', [
                'PerformaInvoice'       => $q,
                'PerformaInvoiceDetail' => $items,
                'TermCondition'   => $terms,
                // if your blade expects $pic, keep a data-uri
                'pic'             => $this->toDataUri($logoUrl)
            ]);

        $downloadName = $q->strPartyName . $q->iPerformaInvoiceNo . '.pdf';
    ...
    /**
     * POST /api/Invoices/{id}/copy
     */
    public function copy($id)
    {
        $user = Auth::user();
    
        $old = PerformaInvoice::where([
            'iStatus' => 1,
            'isDelete' => 0,
        ...
        $last = PerformaInvoice::orderByDesc('performainvoiceId')
        ...
        // ✅ INSERT NEW Invoice
        $copyId = DB::table('performa_invoice')->insertGetId([
            'iYearId'           => $old->iYearId,
            'iPerformaInvoiceNo'      => $nextInvoiceNo,
            'iPartyId'          => $old->iPartyId,
            'iCompanyId'        => $old->iCompanyId,
            'InvoiceValidity' => $old->InvoiceValidity,
        ...
        // ✅ COPY ALL ITEMS
        $details = PerformaInvoiceDetail::where([
            'performainvoiceID' => $id,
            'iStatus'     => 1,
            'isDelete'    => 0
        ])->get();
        ...
        return response()->json([
            'success' => true,
            'message' => 'Performa Invoice copied successfully',
            'new_Invoice_id' => $copyId,
            'new_Invoice_no' => $nextInvoiceNo
        ], 201);
    }
    ...
    public function sendWhatsApp(Request $request, $id)
{
    ...
    $pdfUrl = route('api.employee.Invoices.pdf.link', $id);
