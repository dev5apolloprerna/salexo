...
use App\Models\{PerformaInvoiceDetail, PerformaInvoice, Service};
use Illuminate\Support\Facades\Validator;

class PerformaInvoiceDetailApiController extends Controller
{
    ...
    public function index(Request $request)
{
    ...
    // Items (NO pagination)
    $itemsQuery = PerformaPerformaInvoiceDetail::query()
    ...
    {
        $row = PerformaPerformaInvoiceDetail::query()
            ->from('performa_invoicedetails')
   ...
   public function store(Request $request)
{
    ...
    $validator->after(function ($v) use ($request) {
            ...
            $exists = PerformaPerformaInvoiceDetail::query()
                ->from('performa_invoicedetails')
                ->join('performa_invoice as q', 'performa_invoicedetails.performainvoiceID', '=', 'q.performainvoiceID')
    ...
    $validator->after(function ($v) use ($request) {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Performa Invoice detail updated',
            'id'      => $id,
            'calc'    => [
                'amount'      => round($base, 2),
                'discount'    => $lineDiscount,
     ...
     */
        public function destroy(Request $request)
        {
            ...
            try {
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Performa Invoice detail deleted',
                ], 200);
            } catch (\Throwable $e) {
                DB::rollBack();
