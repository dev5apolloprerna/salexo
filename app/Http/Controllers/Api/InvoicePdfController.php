...
{
public function invoicePdfLink(Request $request, int $id)
{
    try {
        ...
        $template = $this->getDefaultTemplateForCompany($invoice->iCompanyId);
        ...
        $dir = base_path('../public_html/uploads/invoice_pdf');
...
protected function getDefaultTemplateForCompany(int $companyId): ?InvoiceTemplate
{
    ...
    $tpl = InvoiceTemplate::where('guid', $guid)->first();
    if (!$tpl) return null;
...
protected function previewData($invoice): array
    {
       ...
       $partyName  = $party->strPartyName ?? 'Party';
        ...
        $invoiceNumber = $clean($invoice->iInvoiceNo ?? $invoice->iInvoiceNo) ?? ('Invoice-'.$qId);
      
        $invoiceDate   = $fmtDate($invoice->quotationDate ?? $invoice->entryDate, now());
