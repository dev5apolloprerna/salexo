...
class PerformaInvoicePdfController extends Controller
{
public function performainvoicePdfLink(Request $request, int $id)
{
    try {
        ...
        $template = $this->getDefaultTemplateForCompany($performainvoice->iCompanyId);
        ...
        $data['extraTerms'] = DB::table('termcondition')
            ->where([
                'iStatus'   => 1,
                'isDelete'  => 0,
                'companyID' => $performainvoice->iCompanyId,
            ])
        ...
        // 6) Save under /public/uploads/invoice_pdf/
        $safeParty = Str::slug($data['partyName'] ?? 'party');
        $safeNo    = Str::slug($data['invoiceNumber'] ?? ('INVOICE-' . $id));
        $fileName  = "{$safeParty}-{$safeNo}.pdf";

        // use standard public_path so URL and path match
        $dir = base_path('../public_html/uploads/performa_invoice_pdf');
        ...
        // e.g. https://your-domain.com/uploads/invoice_pdf/party-qtn-123.pdf
        $url = url('uploads/performa_invoice_pdf/' . $fileName);

        return response()->json([
            'success' => true,
            'message' => 'Performa Invoice PDF generated.',
...
protected function getDefaultTemplateForCompany(int $companyId): ?PerformaInvoiceTemplate
{
    ...
    $tpl = PerformaInvoiceTemplate::where('guid', $guid)->first();
    if (!$tpl) return null;
...
protected function previewData($performainvoice): array
    {
       ...
       $partyName  = $party->strPartyName ?? 'Party';
        ...
        $performainvoiceNumber = $clean($performainvoice->iInvoiceNo ?? $performainvoice->iInvoiceNo) ?? ('Invoice-'.$qId);
      
        $performainvoiceDate   = $fmtDate($performainvoice->quotationDate ?? $performainvoice->entryDate, now());
        $validTill       = $fmtDate($performainvoice->valid_till ?? $performainvoice->invoiceValidity, now()->addDays(7));

        /* -----------------  Footer  ----------------- */
        $paymentTerms = $clean($performainvoice->paymentTerms) ?? '-';
        $delivery     = $clean($performainvoice->deliveryTerm) ?? '-';
        $modeOfDespatch = $clean($performainvoice->modeOfDespatch) ?? '';
        $warranty     = $clean($performainvoice->warranty) ?? '-';
        $extraTerms     = $clean($performainvoice->strTermsCondition) ?? '';
        ...
        return [
            ...
            'quotationDate'  => $performainvoiceDate,
            'validTill'      => $validTill,
