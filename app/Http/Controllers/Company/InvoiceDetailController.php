...
class InvoiceDetailController extends Controller
{
    ...
    public function checkDuplicate(Request $request)
        {
            ...
            $detailId    = $request->invoicedetailsId;       // dd-mm-YYYY or yyyy-mm-dd
    
            $exists = InvoiceDetail::query()
                        ->join('invoice as q', 'invoicedetails.invoiceID', '=', 'q.invoiceID')
                        ->where('invoicedetails.invoiceID', $request->invoiceID)   // from details
                        ...
                        ->when($detailId, function($q) use($detailId) {
                                    return $q->where('invoicedetails.invoicedetailsId','!=', $detailId);
                                })
                                      // from details
                        ->where([
    ...
    public function index(Request $request, $id)
    {
        ...
        $InvoiceDetail = InvoiceDetail::query()
            ...
            ->orderBy('invoicedetails.invoicedetailsId', 'ASC')
            ->paginate(10);

        // Header row
        $CompanyName = DB::table('invoice')
        ...
        $allRows = DB::table('invoicedetails')
            ->where([
                'invoiceID' => $id,
                'iStatus'     => 1,
                'isDelete'    => 0,
        ...
        return view('company_client.invoicedetail.index', compact(
    ...
    public function create(Request $request)
    {
        ...
        $header = DB::table('invoice')
            ->where('invoiceId', (int) $data['invoiceID'])
    ...
    //dd($lineDiscount);
        ...
        if (($header->discount_type ?? '') === 'amount' && (float)$header->discount > 0) {
            $this->reallocateAmountDiscount((int)$data['invoiceID'], (float)$header->discount);
        }

        return redirect()
            ->route('invoicedetails.index', [$data['invoiceID']])
    ...
    // QuotationDetailController.php
        ...
        {
            $row = InvoiceDetail::where('invoicedetailsId', $id)->firstOrFail();
   ...
   public function update(Request $request, $Id)
    {
        ...
        $row = DB::table('invoicedetails')->where([
            'invoicedetailsId' => (int)$data['invoicedetailsId'],
            'isDelete'           => 0,
        ])->first();
    ...
    {
        // 1) Find the row (need invoiceID)
        $row = DB::table('invoicedetails')
            ->where(['invoicedetailsId' => (int)$Id, 'isDelete' => 0])
            ->select('invoiceID')
            ->first();
