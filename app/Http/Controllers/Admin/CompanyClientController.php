...
class CompanyClientController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $companyIds = $clients->getCollection()->pluck('company_id');

            $employees = Employee::where('isCompanyAdmin', 1)->get();
            $activeLeadCounts = LeadMaster::active()
                ->whereIn('iCustomerId', $clients->pluck('company_id'))
                ->selectRaw('iCustomerId, COUNT(*) as aggregate')
                ->groupBy('iCustomerId')
                ->pluck('aggregate', 'iCustomerId');
                
            
            // Assign total leads to each client
            foreach ($clients as $client) {
                $client->active_leads = (int) $activeLeadCounts->get($client->company_id, 0);
            }

            return view('admin.company_client.index', compact('clients', 'search'));
        } catch (\Exception $e) {
