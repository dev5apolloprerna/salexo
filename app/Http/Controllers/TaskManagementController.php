...
class TaskManagementController extends Controller
{
    public function index()
    {
        ...
        if (!$isCompanyAdmin) {
            $tasksQuery->where('assigned_employee_id', $user->emp_id);
        }

        $tasks = $tasksQuery->latest()->get();

        return view('company_client.tasks.index', compact('employees', 'tasks', 'isCompanyAdmin'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'assigned_employee_id' => [
                'required',
                Rule::exists('employee_master', 'emp_id')->where(function ($query) use ($user) {
