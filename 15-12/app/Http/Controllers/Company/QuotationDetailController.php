...
class QuotationDetailController extends Controller
{
    public function serviceLookup(Request $request)
    {
        ...
        $rows = Service::query()
            ...
            ->where('company_id', $user->company_id) // if you have this flag
    ...
    public function index(Request $request, $id)
    {
        ...
        $Product = Service::where(['iStatus' => 1, 'isDelete' => 0])->where('company_id', $user->company_id)
        ...
        $allRows = DB::table('quotationdetails')
            ->where([
                'quotationID' => $id,
                'iStatus'     => 1,
                'isDelete'    => 0,
        ...
        return view('company_client.quotationdetail.index', compact(
            'QuotationDetail', 'id', 'CompanyName', 'Product', 'summary'
        ));
    }
    ...
    private function resolveServiceIdForRequest(Request $request): int
{
    ...
    public function applyDiscount(Request $request, $quotationId)
    {
        $data = $request->validate([
        ...
        if ($items->isEmpty()) {
            return back()->with('error', 'No items found for this quotation.');
        }
